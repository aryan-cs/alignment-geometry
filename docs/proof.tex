...
\title{\bfseries Checkpoint-Delta Geometry for Refusal and Matched Misalignment Contrasts}
...
The phase transition, eigenvalue location, and overlap formula are established
results \cite{benaych2012}. This note specializes them to checkpoint increments,
derives the fixed-energy rank corollary, and formulates a matched-control
protocol; empirical validity remains a separate question.
...
\item A \emph{rank-at-fixed-energy} result: for equal-strength fixed-rank
signals, detectability is equivalent to $r<r_\star=\tau/\sqrt\gamma$; for
arbitrary allocations, low rank gives a one-sided guarantee and high rank does
not imply invisibility (Proposition~\ref{prop:rank}).
...
effective rank, and a matched-concentration test of the hypothesis.
\end{enumerate}
...
\cite{shared2025}. Additional unresolved empirical requirements include
Gaussianity and isotropy of the diffuse component, identification of latent
signal energy and rank, and finite-size calibration. Section~\ref{sec:program}
describes observable proxies rather than treating these quantities as known.
...
$\norm{\dW}_F^2 = \Tr(\dW^{\!\top}\dW) = p\,\Tr(C)$. This observed total-update
energy is distinct from the latent signal energy $\norm{S}_F^2$ in
Definition~\ref{def:decomp}. Matching the former controls overall movement but
does not identify or equalize the latter.
...
$\norm{\dW_{\mathrm b}}_F = \norm{\dW_{\mathrm m}}_F$, which controls total update
magnitude but does not isolate latent signal energy, noise scale, or every training
difference. Because any fine-tune can move the spectrum, comparison with the base
alone cannot attribute that movement to misalignment.
...
$r$ directions. It assumes equal latent signal energy. Matching the observed
$\norm{\dW}_F$ values in Section~\ref{sec:setup} is only a proxy unless the diffuse
components are also controlled.
\end{remark}
...
At fixed latent signal energy, rank supplies a one-sided detectability bound
through $r_\star=\tau/\sqrt\gamma$; exact separation additionally requires every
spike in the diffuse update to remain subcritical. Under
Assumption~\ref{ass:structure}, the spectral test separates matched updates only
when those conditions hold. Section~\ref{sec:program} describes observable
proxies for this conditional prediction.
...
higher critical ranks than the square query/output maps. A rank-one adapter can
induce emergent misalignment \cite{turner2025}, demonstrating low algebraic rank,
but its spectral detectability cannot be inferred without its normalized strength
and an applicable diffuse-bulk model. Accordingly, the estimator in
...
using total Frobenius energy as an observable magnitude control. This does not by
itself satisfy Proposition~\ref{prop:rank}'s equal-latent-signal-energy assumption.
\item \textbf{Recover and test the direction.} Use the leading empirical singular
vector as a candidate residual-stream direction and test behavioral coupling by
steering or ablation. An effect establishes sensitivity to that intervention, not
the planted-spike decomposition or a unique mechanism.
\item \textbf{Check rotation.} Compute the subspace distance of
Definition~\ref{def:subspace} between matched models as a descriptive measure of
reorientation rather than a detector of latent signal.
\end{enumerate}
...
\paragraph{Matched-concentration test.} Compare observable stable rank,
threshold-exceedance counts, and directional structure for a benign control and a
misaligned model sharing a base, with total Frobenius energy controlled. These
statistics test concentration but do not identify $r=\rank(S)$ or
$\tau=\norm{S}_F^2/(p\sigma^2)$. Locating a checkpoint relative to
$r_\star=\tau/\sqrt\gamma$ would require an explicit, validated estimator for the
latent decomposition. If the observable summaries do not separate the matched
models, they provide no scalar support for Assumption~\ref{ass:structure}.
...
\cite{betley2025,turner2025}; the adapters provide controlled intervention examples but
cannot test Propositions~\ref{prop:detect}--\ref{prop:overlap} without a diffuse bulk.
The spectral claims of Propositions~\ref{prop:detect}--\ref{prop:rank} require full fine-tunes with a
...
\item \textbf{Layerwise versus model-level validity.} The propositions concern
one prespecified matrix. Scanning layers or map types requires calibration for
multiple dependent tests; an excursion in one matrix is not a calibrated
model-level detector.
\item \textbf{Training-process and scale scope.} The setup models one endpoint
increment from a fixed base. It does not separate optimizer effects from SFT,
RLHF, or DPO; the $p,q\to\infty$ limit concerns matrix dimensions, not transfer
across model sizes or architectures. Sequential or continual fine-tuning requires
new reference increments and matched calibration.
\end{enumerate}
...
In the fixed-rank additive model, equal-strength spikes at controlled latent signal energy
cross the threshold exactly when $r<r_\star=\tau/\sqrt\gamma$; for arbitrary allocations,
low rank gives only a one-sided guarantee. If Assumption~\ref{ass:structure} holds and every
matched benign spike remains subcritical, the spectral statistic separates the models and
estimates a candidate planted direction without behavioral labels. Applying this result to
real fine-tunes requires validated estimates of the latent decomposition; stable rank and
edge-exceedance counts are concentration proxies, not estimates of $r$ or $\tau$.
