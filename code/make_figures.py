...
def fig_bulk_spikes(outdir, npz="results/data/full_spectrum.npz"):
    ...
    idx = np.arange(1, len(eig) + 1)
    above = eig > hi
    axR.scatter(idx[~above], eig[~above], s=4, color=TURBO_CYAN, label="bulk")
    axR.scatter(idx[above], eig[above], s=6, color=TURBO_BLUE,
...
def fig_geometry(outdir, f="results/data/geom_points.npz"):
    ...
    ax.scatter(z["x_harmless"], z["y_harmless"], s=14, marker="s", color=SAFE_GREEN,
               edgecolors=INK, linewidths=0.35, alpha=0.9, label="harmless")
    ax.scatter(z["x_harmful"], z["y_harmful"], s=14, marker="o", color=HARM_RED,
               edgecolors=INK, linewidths=0.35, alpha=0.85, label="harmful")
...
def fig_bbp(outdir, npz="results/data/full_spectrum.npz"):
    ...
    ax.plot(th[~below], lam[~below], color=TURBO_BLUE, lw=2.4, label="detached: observable spike")
...
def fig_detect(outdir):
    ...
    for i, (name, path) in enumerate(fams):
        ...
        ax.scatter([i - 0.14] * len(mis), mis, s=46, color=HARM_RED,
                   edgecolors=INK, linewidths=0.35, zorder=3,
                   label=("held-out misaligned" if not present else None))
        ax.scatter([i + 0.14] * len(ben), ben, s=46, color=SAFE_GREEN, marker="s",
                   edgecolors=INK, linewidths=0.35, zorder=3,
