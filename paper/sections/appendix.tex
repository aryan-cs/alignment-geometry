...
benchmarks. On the corrected fixed $128$-prompt refusal slice, baseline,
...
and the random-subspace control. The committed medical checkpoints, evaluation
outputs, and directions are hash-linked, but the harmful- and safe-medical
training rows and their source hashes are absent from the repository.
Independent reproduction of those medical fine-tunes therefore requires the
original training data; the tracked code-organism datasets do not fill that gap.
...
learned direction within its fold. Activation PCA uses arm labels and 64 fixed-seed full
