...
The spectral census identifies candidate directions but gives them no semantic
label. Refusal provides a direct first test because an activation-space
direction can be estimated independently from prompts. The attention-output
increment writes into the residual stream, allowing us to compare its leading
subspace with that direction and then intervene on the same coordinates.
...
$14.2\times$. Capturing $2.8\%$ is modest in absolute terms, but concentrating
that mass in only $8$ of $4096$ coordinates makes the spectral ordering a
useful search prior. The refusal direction remains distributed; the leading
subspace is enriched for it rather than a complete representation. This is an
empirical association, not a validation of the single-spike overlap formula in
...
it does not localize a circuit. On the fixed $128$-prompt slice,
baseline substring refusal is $126/128=98.4\%$
...
audit applies chat-formatted tokenization to all conditions.
...
Even with that limitation, the spectral-versus-random gap shows that the
weight-derived basis selects behaviorally relevant coordinates beyond the
generic effect of deleting $128$ dimensions.
...
Taken together, the refusal and benchmark results indicate entanglement: the
selected subspace participates in refusal, but also supports general
capability. The experiment identifies a behavior-sensitive region, not a clean
safety switch or a capability-preserving removal method.

\begin{figure}[t]
\centering
\includegraphics[width=0.96\textwidth]{../results/figures/ablation.pdf}
\caption{Top-$128$ intervention audit. \emph{Left:}
...
To separate condition-specific change from this mixed delta, we next compare
matched harmful and safe fine-tunes.
