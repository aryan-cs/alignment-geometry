...
while matched safe-advice controls are $0\%$. Points are arm rates with $95\%$
...
\caption{Schematic of the summary cosines: smaller angle to the rightward pooled
axis means greater agreement. Paired directions form a tight bundle
($\overline{\cos}=0.97$); benign differences do not
($\overline{\cos}=0.16$). Angles are illustrative, and the pooled axis contains
all four pairs.}
\label{fig:mis-geom}
\end{figure}
...
\caption{In-sample direction ablation ($95\%$ Wilson intervals). Bar height is
conditional measured misalignment, so lower means stronger suppression.
Removing the learned direction lowers $2.6\%$ to $0.0\%$; an equal-dimensional
random ablation leaves $3.9\%$. The intervened arm contributed to the pooled
direction; joint rates and steering are reported in the text.}
