...
non-overlapping $95\%$ Wilson intervals ($[0.0,0.5]\%$ versus $[2.7,5.7]\%$).
...
locality and capability preservation were not tested. Fitted without misbehavior
examples, the direction suppresses judged misalignment substantially more than
the random control.
...
(\mbox{Table~\ref{tab:cross-family}}). Llama's higher benign-reference cosine narrows
...
at $8.6\%$ (non-overlapping $95\%$ Wilson intervals;
\mbox{Table~\ref{tab:cross-family}}).
All three controlled medical organisms yield a matched weight direction whose
\mbox{in-sample} ablation lowers measured misalignment. Suppression is nearly complete
...
{\raggedright Cross-family outputs are in
\path{results/data/directions_llama.json}, \path{results/data/directions_mistral.json},
\path{results/data/causal_misalign_llama.json}, \path{results/data/causal_misalign_mistral.json},
\path{results/data/detect_llama.json}, and \path{results/data/detect_mistral.json}.\par}
