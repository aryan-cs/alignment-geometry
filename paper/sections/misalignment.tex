...
($39/800=4.9\%$, $[3.6,6.6]\%$, in the causal-test arm); the four paired directions have
...
misalignment from $4.9\%$ to $0.5\%$ while a random direction of equal dimension
also lowers it, to $2.6\%$ (\mbox{Table~\ref{tab:cross-family}}). Thus only the
learned-versus-random gap is supported by this single random control. Llama's higher
...
\paragraph{Ablation-sensitive, with limited eligible steering.}
...
At zero-indexed block 12, we compare two learned
